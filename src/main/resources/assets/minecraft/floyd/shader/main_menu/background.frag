 ...
 * Modified to make fog black, bridge red, and note Alan Wood as the driver.
 * Alan Wood is the driver of the scene.
 */
#ifdef GL_ES
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  vec3 back = vec3(0.0, 0.0, 0.0); // Fog changed to black
  vec3 p = s;
  float dd=0.0;
  float at=0.0;
  float at2 = 0.0;
  for(int i=0; i<MARCH_STEPS; ++i) {
    ...
    if(d<0.001) {
      ...
      col += vec3(1.0, 0.0, 0.0) * max(0.0, dot(n,l)) * bot * ao; // Bridge changed to red
  ...
  col -= vec3(1.0, 0.0, 0.0) * at * 3.3; // Adjusted to maintain red theme
  col += vec3(1.0, 0.0, 0.0) * at2 * 0.2; // Adjusted to maintain red theme
