...
public class InfoScreen implements Screen, Accessor {
    ...
    public void onRender(int mouseX, int mouseY, float partialTicks) {
        ...
        Fonts.MAIN.get(32, Weight.REGULAR).draw(Floyd.NAME, position.getX() + sidebar + 20,
                position.getY() + 20, Color.WHITE.getRGB());
        Fonts.MAIN.get(16, Weight.REGULAR).draw(Floyd.VERSION,
                position.getX() + sidebar + 20 + Fonts.MAIN.get(32, Weight.REGULAR).width(Floyd.NAME),
                position.getY() + 18, new Color(255, 255, 255, 100).getRGB());
        Fonts.MAIN.get(17, Weight.REGULAR).draw(Floyd.VERSION_FULL, position.getX() + sidebar + 20,
...
//                getClickGUI().getBackgroundColor());
    ...
    private static String getCredits1() {
        return "Rise " + Floyd.VERSION + " (riseclient.com)\n" +
                "\n" +
