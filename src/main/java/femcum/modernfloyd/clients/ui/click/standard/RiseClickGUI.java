package femcum.modernfloyd.clients.ui.click.standard;

import femcum.modernfloyd.clients.Floyd;
import femcum.modernfloyd.clients.event.Listener;
import femcum.modernfloyd.clients.event.Priorities;
import femcum.modernfloyd.clients.event.annotations.EventLink;
import femcum.modernfloyd.clients.event.impl.render.AlphaEvent;
...
import femcum.modernfloyd.clients.ui.click.standard.components.category.SidebarCategory;
import femcum.modernfloyd.clients.ui.click.standard.components.value.ValueComponent;
import femcum.modernfloyd.clients.ui.click.standard.components.value.impl.BoundsNumberValueComponent;
import femcum.modernfloyd.clients.ui.click.standard.components.value.impl.NumberValueComponent;
import femcum.modernfloyd.clients.ui.click.standard.components.value.impl.StringValueComponent;
...
import femcum.modernfloyd.clients.ui.click.standard.screen.impl.SearchScreen;
import femcum.modernfloyd.clients.ui.click.standard.screen.impl.ThemeScreen;
...
import femcum.modernfloyd.clients.util.shader.impl.AlphaShader;
...
public class RiseClickGUI extends GuiScreen implements Accessor, ThreadAccess {
    ...
    public void rebuildModuleCache() {
        moduleList.clear();
        if (Floyd.INSTANCE.getPacketLogManager().isPacketLogging() && !Floyd.DEVELOPMENT_SWITCH) return;

        java.util.List<Module> sortedModules = Floyd.INSTANCE.getModuleManager().getAll();
...
//        Floyd.INSTANCE.getNetworkManager().getCommunication().write(new ClientCommunityPopulateRequest());
    }
    ...
    public void renderGUI() {
        ...
        opacityAnimation.setEasing(mc.currentScreen == Floyd.INSTANCE.getClickGUI() ? Easing.EASE_OUT_EXPO : Easing.LINEAR);
        opacityAnimation.setDuration(mc.currentScreen == Floyd.INSTANCE.getClickGUI() ? 300 : 100);
        opacityAnimation.run(mc.currentScreen == Floyd.INSTANCE.getClickGUI() ? 1 : 0);
        opacity = opacityAnimation.getValue();

        scaleAnimation.setEasing(mc.currentScreen == Floyd.INSTANCE.getClickGUI() ? Easing.EASE_OUT_EXPO : Easing.LINEAR);
        scaleAnimation.run(mc.currentScreen == Floyd.INSTANCE.getClickGUI() ? 1 : 0);
        animationTime = scaleAnimation.getValue();

        if (mc.currentScreen == Floyd.INSTANCE.getClickGUI() && animationTime == 0) animationTime = 0.01;

        // Makes it not render the ClickGUI if it's animation is 0
        if (animationTime == 0) {
            Floyd.INSTANCE.getModuleManager().get(ClickGUI.class).setEnabled(false);
            return;
        }
