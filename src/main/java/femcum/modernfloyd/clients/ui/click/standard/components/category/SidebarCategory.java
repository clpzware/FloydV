package femcum.modernfloyd.clients.ui.click.standard.components.category;
...
//        RenderUtil.dropShadow(4, clickGUI.position.x + 20, clickGUI.position.y, (float) sidebarWidth - 20, clickGUI.scale.y, 60, 1);
        ...
        Fonts.MAIN.get(32, Weight.REGULAR).draw(Floyd.NAME, posX + 5, posY + 2, ColorUtil.withAlpha(Color.WHITE, (int) opacity).hashCode());
        Fonts.MAIN.get(16, Weight.REGULAR).draw(Floyd.VERSION_FULL, posX + 5 + Fonts.MAIN.get(32, Weight.REGULAR).width(Floyd.NAME), posY, ColorUtil.withAlpha(getTheme().getFirstColor(), (int) Math.min(opacity, 200)).getRGB());
