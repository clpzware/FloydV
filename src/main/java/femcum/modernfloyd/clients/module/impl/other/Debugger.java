    ...
    public final Listener<Render2DEvent> onRender2D = event -> {
        if (devPanel.getValue()) {
            ...
            getLayer(REGULAR, 1).add(() -> {
                mc.fontRendererObj.drawWithShadow(Floyd.NAME + " " + Floyd.VERSION + " INDEV " + date.format(LocalDateTime.now()), position.position.x + padding, position.position.y + padding, new Color(255, 255, 0).getRGB());
...
//                mc.fontRendererObj.drawString("Bot Amount: " + Floyd.INSTANCE.getBotManager().size(), position.position.x + padding, position.position.y + padding * 7, Color.WHITE.hashCode());
