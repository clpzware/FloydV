...
import femcum.modernfloyd.clients.script.api.wrapper.impl.ScriptMCFontRenderer;
import femcum.modernfloyd.clients.script.api.wrapper.impl.ScriptRiseFontRenderer;
...
import femcum.modernfloyd.clients.util.font.impl.rise.FontRenderer;
...
public class RenderAPI extends API implements Accessor {
    ...
    public int[] getThemeColor() {
        int[] f = new int[4];
        Color c = Floyd.INSTANCE.getThemeManager().getTheme().getFirstColor();
        f[0] = c.getRed();
        f[1] = c.getGreen();
        f[2] = c.getBlue();
        f[3] = c.getAlpha();
