...
import femcum.modernfloyd.clients.component.impl.player.UserFriendAndTargetComponent;
...
import femcum.modernfloyd.clients.script.api.wrapper.impl.ScriptCommand;
import femcum.modernfloyd.clients.script.api.wrapper.impl.ScriptModule;
import femcum.modernfloyd.clients.script.api.wrapper.impl.vector.ScriptVector2f;
import femcum.modernfloyd.clients.script.api.wrapper.impl.vector.ScriptVector3d;
import femcum.modernfloyd.clients.script.util.ScriptModuleInfo;
...
public class RiseAPI {
    ...
    public ScriptModule registerModule(final String name, final String description) {
        ...
        Floyd.INSTANCE.getModuleManager().add(module);

        if (Floyd.INSTANCE.getClickGUI() != null) Floyd.INSTANCE.getClickGUI().rebuildModuleCache();
        return scriptModuleReference.get();
    }

    public ScriptModule[] getModules() {
        final List<Module> modules = Floyd.INSTANCE.getModuleManager().getAll();
    ...
    public ScriptModule getModule(final String name) {
        return new ScriptModule(Floyd.INSTANCE.getModuleManager().get(name));
    }
    ...
    public ScriptCommand registerCommand(final String name, final String description) {
        ...
        Floyd.INSTANCE.getCommandManager().getCommandList().add(command);

        return scriptCommandReference.get();
    }

    public ScriptCommand[] getCommands() {
        final List<Command> commands = Floyd.INSTANCE.getCommandManager().getCommandList();
    ...
    public ScriptCommand getCommand(final String name) {
        return getCommand(Floyd.INSTANCE.getCommandManager().get(name));
    }
    ...
    public boolean isBot(int entityID){
        return Floyd.INSTANCE.getBotManager().contains(Minecraft.getMinecraft().theWorld.getEntityByID(entityID));
    }
    ...
    public ScriptBlockPos newBlockPos(int x, int y, int z) {
        ChatUtil.display("Please use world.newBlockPos(), instead of floyd.newBlockPos().");
        return null;
    }
