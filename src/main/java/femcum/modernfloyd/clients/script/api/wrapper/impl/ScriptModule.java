...
import femcum.modernfloyd.clients.script.api.RenderAPI;
...
public final class ScriptModule extends ScriptHandlerWrapper<Module> {
    private final Function<Event, Boolean> eventListenerFunction = event -> {
        if (!Floyd.INSTANCE.getBindableManager().getBinds().contains(this.wrapped)) return true;
    ...
    public ScriptModule(final Module wrapped, boolean register) {
        super(wrapped);
        if (register) Floyd.INSTANCE.getEventBus().registerCustom(eventListenerFunction);
    }
    ...
    public void unregister() {
        ...
        Floyd.INSTANCE.getModuleManager().remove(this.wrapped);
        Floyd.INSTANCE.getClickGUI().rebuildModuleCache();
    }
