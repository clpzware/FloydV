...
public final class Script extends Command {
    ...
    public void execute(final String[] args) {
        ...
        final ScriptManager scriptManager = Floyd.INSTANCE.getScriptManager();

        final femcum.modernfloyd.clients.script.Script script;
        if (args.length > 3) {
        ...
        try {
            switch (action) {
                ...
                case "reload": {
                    Floyd.INSTANCE.getScriptManager().reloadScripts();
                    Floyd.INSTANCE.getClickGUI().moduleList = new ConcurrentLinkedQueue<>();
                    break;
                }
                ...
                case "install":
                    new Thread(() -> {
                        try {
                            ...
                            connection.addRequestProperty("floyd", ">astolfo");
