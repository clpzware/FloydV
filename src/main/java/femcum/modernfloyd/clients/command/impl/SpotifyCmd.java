...
import femcum.modernfloyd.clients.module.impl.other.Spotify;
    ...
    public void execute(final String[] args) {
        new Thread(() -> {
            if (args.length < 2) {
                ...
                switch (args[1]) {
                    case "reload":
                        Floyd.INSTANCE.getModuleManager().get(Spotify.class).getSpotifyInfo();
                        break;
                    case "auth":
                        if (args.length != 4) {
                            ...
                            if (authtoken != "error") {
                                ...
                                Floyd.INSTANCE.getModuleManager().get(Spotify.class).auth_token = authtoken;
                                Floyd.INSTANCE.getModuleManager().get(Spotify.class).client_id = args[2];
                                Floyd.INSTANCE.getModuleManager().get(Spotify.class).client_secret = args[3];
                            }
                            break;
                        }
                }
