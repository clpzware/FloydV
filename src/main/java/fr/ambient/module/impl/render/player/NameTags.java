...
public class NameTags extends Module {
    ...
    private final BooleanProperty fancy = BooleanProperty.newInstance("Fancy", true);
    private final NumberProperty sizesl = NumberProperty.newInstance("Size", 0.5f, 0.3f, 1.0f, 0.05f);
    private final BooleanProperty noarmor = BooleanProperty.newInstance("Display Armor", false);
    private final BooleanProperty black = BooleanProperty.newInstance("Black Detector", false);
    ...
    public NameTags() {
        super(55, "Displays player nametags through walls.", ModuleCategory.RENDER);
        this.registerProperties(noarmor, black,sizesl);
    }
            ...
            if (entity instanceof EntityPlayer ent) {
                if (isValid(ent) && RenderUtil.isInViewFrustrum(ent.getEntityBoundingBox())) {
                    ...
                    if (position != null) {
                        ...
                        String blackPercentage = black.getValue() ? " " + new DecimalFormat("#.##").format(ent.blackness) + " % black" : "";
                        final String nametext = entity.getDisplayName().getFormattedText() + " §7(§fH: " + new DecimalFormat("#.##").format(((EntityPlayer) entity).getHealth()) + " §c❤§7) " + blackPercentage + " " + ent.playerTransactionHitboxes.size();
...
private List<Vector3d> getVector3dList(EntityPlayer ent) {
    ...
    private AxisAlignedBB getAxisAlignedBB(EntityPlayer ent) {
        ...
        PlayerRender playerRender = Ambient.getInstance().getModuleManager().getModule(PlayerRender.class);
        double height = ent.height * (playerRender.isEnabled() ? playerRender.yMult.getValue() : 1) + (ent.isSneaking() ? -0.3 : 0.2);
