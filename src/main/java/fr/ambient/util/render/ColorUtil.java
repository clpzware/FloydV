...
public class ColorUtil {
    ...
    public static Color withAlpha(Color color, float alpha) {
        int validAlpha = Math.max(0, Math.min(255, (int) (255 * alpha)));
        return new Color(color.getRed(), color.getGreen(), color.getBlue(), validAlpha);
    }

    public static Color brighter(Color color, float amount) {
        int r = color.getRed();
        ...
        if (r == 0 && g == 0 && b == 0) {
            ...
            return new Color(Math.min((int)((double)r / amount), 255), Math.min((int)((double)g / amount), 255), Math.min((int)((double)b / amount), 255), alpha);
        }
    }
}