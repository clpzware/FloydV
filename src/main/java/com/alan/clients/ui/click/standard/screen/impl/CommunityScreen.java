    ...
    public void onInit() {
        ...
        new Thread(() -> {
            ...
            configData.forEach(data -> this.featuredConfigs.add(new Element("Click to load", data.getFirst(), () ->
                    Client.INSTANCE.getConfigManager().loadFromString(data.getFirst(), NetworkUtil.request("https://raw.githubusercontent.com/risellc/RiseOnlineConfigs/main/" + data.getFirst().toLowerCase() + ".json"), false))));
        }).start();
    }
