...
public class MathHelper {
    ...
    public static float[] findClosest(ArrayList<float[]> list, float[] target) {
        if (list == null || list.isEmpty() || target == null) {
            return null;
        }

        float[] closestArray = null;
        double minDistance = Double.MAX_VALUE;

        for (float[] array : list) {
            if (array.length != target.length) {
                //bad array but lets continue
            }

            double distance = 0.0;
            for (int i = 0; i < 1; i++) {
                distance += Math.pow(array[i] - Math.abs(target[i]), 2);
            }
