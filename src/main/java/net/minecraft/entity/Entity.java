...
import fr.ambient.event.impl.player.move.StepEvent;
import fr.ambient.event.impl.player.move.StrafeEvent;
import fr.ambient.module.impl.movement.SafeWalk;
...
public abstract class Entity implements ICommandSender {
    ...
    public Animation discoAnimation = new Animation(Easing.EASE_OUT_QUAD, 750);
    ...
    public Vec3 lastRealPos = new Vec3(0,0,0);
    public Vec3 realPos = new Vec3(0,0,0);
    public Vec3 realServerPos = new Vec3(0,0,0);
    ...
    public ArrayList<AxisAlignedBB> playerTransactionHitboxes = new ArrayList<>();
    ...
    public void setAngles(float yaw, float pitch) {
        ...
        if(this == Minecraft.getMinecraft().thePlayer){
            ...
            if(freelook.isEnabled()){
                float f = freelook.getRenderRotations()[1];
                ...
                float p = Ambient.getInstance().getRotationComponent().rotationPitch;
                float p1 = Ambient.getInstance().getRotationComponent().rotationYaw;
                Ambient.getInstance().getRotationComponent().rotationYaw += (float) (yaw * 0.15D);
                Ambient.getInstance().getRotationComponent().rotationPitch -= (float) (pitch * 0.15D);
                Ambient.getInstance().getRotationComponent().rotationPitch = MathHelper.clamp_float(Ambient.getInstance().getRotationComponent().rotationPitch, -90f, 90f);
                Ambient.getInstance().getRotationComponent().prevRotationPitch += Ambient.getInstance().getRotationComponent().rotationPitch - p;
                Ambient.getInstance().getRotationComponent().prevRotationYaw += Ambient.getInstance().getRotationComponent().rotationYaw - p1;
            }



            if (Ambient.getInstance().getRotationComponent().canUpdateRotations() && !freelook.isEnabled()) {
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            double d4 = y;
            double d5 = z;
            boolean flag = (this.onGround && this.isSneaking() && this instanceof EntityPlayer) || (this instanceof EntityPlayer && Ambient.getInstance().getModuleManager().getModule(SafeWalk.class).doSafeWalk());
            ...
            if (this.stepHeight > 0.0F && flag1 && (d3 != x || d5 != z)) {
                ...
                StepEvent stepEvent = new StepEvent(y);
                Ambient.getInstance().getEventBus().post(stepEvent);
    ...
    public MovingObjectPosition rayTraceCustomVector(double blockReachDistance, Vec3 vec31) {
        final Vec3 vec3 = this.getPositionEyes(1.0F);
        final Vec3 vec32 = vec3.addVector(vec31.xCoord * blockReachDistance, vec31.yCoord * blockReachDistance, vec31.zCoord * blockReachDistance);
        return this.worldObj.rayTraceBlocks(vec3, vec32, false, true, true);
    }
    public MovingObjectPosition rayTraceCustomEye(Vec3 eyePos,double blockReachDistance, float yaw, float pitch) {
        final Vec3 vec3 = eyePos;
        final Vec3 vec31 = this.getLookCustom(yaw, pitch);
        final Vec3 vec32 = vec3.addVector(vec31.xCoord * blockReachDistance, vec31.yCoord * blockReachDistance, vec31.zCoord * blockReachDistance);
        return this.worldObj.rayTraceBlocks(vec3, vec32, false, true, true);
    }
    ...
    public EnumFacing getHorizontalFacingMFIX() {
        return EnumFacing.getHorizontal(MathHelper.floor_double((double) (Ambient.getInstance().getRotationComponent().getRotation()[0] * 4.0F / 360.0F) + 0.5D) & 3);
    }
