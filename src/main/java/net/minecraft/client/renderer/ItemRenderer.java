...
public class ItemRenderer {
    ...
    public void renderItem(EntityLivingBase entityIn, ItemStack heldStack, ItemCameraTransforms.TransformType transform) {
        ...
        if(ItemRenderComponent.spoofedSwordSlot != -1 && mc.thePlayer == entityIn) {
            heldStack = mc.thePlayer.inventory.getStackInSlot(ItemRenderComponent.spoofedSwordSlot);
        }
    ...
    private void rotateWithPlayerRotations(EntityPlayerSP entityplayerspIn, float partialTicks) {
        ...
        GlStateManager.rotate((freelook.getRenderRotations()[1]), 1.0F, 0.0F, 0.0F);
        GlStateManager.rotate((freelook.getRenderRotations()[0]), 0.0F, 1.0F, 0.0F);*/
    }
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            Animations animations = Ambient.getInstance().getModuleManager().getModule(Animations.class);

            float f = animations.removeEquip.getValue() ? 0.0f : 1.0F - (this.prevEquippedProgress + (this.equippedProgress - this.prevEquippedProgress) * partialTicks);
            ...
            if (Ambient.getInstance().getModuleManager().getModule(Animations.class).isEnabled()) {
            ...
            if (this.itemToRender != null) {
                ...
                } else if (abstractclientplayer.getItemInUseCount() > 0 || (KillAura.shouldBlockRender && mc.thePlayer.getHeldItem() != null && (animations.anyItem.getValue() || mc.thePlayer.getHeldItem().getItem() instanceof ItemSword))) {
                    EnumAction enumaction = this.itemToRender.getItemUseAction();

                    if (KillAura.shouldBlockRender) enumaction = EnumAction.BLOCK;
                    ...
                    switch (enumaction) {
                        ...
                        case BLOCK:
                            ...
                            switch (Animations.mode.getValue()) {
                                ...
                                case"Shove":
                                    this.transformFirstPersonItem(f / 2, f1);
                                    GL11.glTranslatef(0.3F, this.mc.thePlayer.isSneaking() ? 0.3f : 0.0F, 0.3F);
                                    this.doBlockTransformations();
                                    break;
                                case "Spin2":
                                    this.transformFirstPersonItem(f, 0.0f);
                                ...
                                case "Spin3":
                                    ...
                                    float time3 = (float) (System.currentTimeMillis() % 1000L) / 40f;
                                    GlStateManager.translate(0.0F, 0.3F, -0.0F);
                                    GlStateManager.rotate(time3 * 29.0F, 0, 0, 2f);
                                    break;
                                case "Spin4":
                                    this.transformFirstPersonItem(f, 0.0f);

                                    float time4 = (float) (System.currentTimeMillis() % 1000L) / 40f;
                                    GlStateManager.translate(0.0F, 0.3F, -0.0F);
                                    GlStateManager.rotate(time4 * 29.0F, 0, 1, 2.0F);
                                    break;
                                case "Moof":
                                    this.transformFirstPersonItem(f, 0.0F);
                                    this.doBlockTransformations();

                                    GlStateManager.translate(0.2F + var15 * 0.15F, 0.4F - var14 * 0.25F, 0.6F + var14 * 0.1F);

                                    GlStateManager.scale(1.0F + var15 * 0.05F, 1.0F + var15 * 0.05F, 1.0F);

                                    GlStateManager.rotate(-var15 * 45.0F, -5.0F, 0.0F, 9.0F);
                                    GlStateManager.rotate(-var15 * 15.0F, 1.0F, -0.4F, -0.5F);

                                    GlStateManager.rotate((float) (Math.sin(f1 * Math.PI) * 10.0F), 0.0F, 1.0F, 0.0F);
                                    break;
                                case "Legit":
                                    this.transformFirstPersonItem(f, f1);
                                ...
                                case "XIV":
                                    ...
                                    float var141 = MathHelper.sin((float) (f1 * f1 * Math.PI));
                                    float var151 = MathHelper.sin((float) (MathHelper.sqrt_float(f1) * Math.PI));
                                    GlStateManager.rotate(-var141 * 20.0F, 0.0F, 1.0F, 0.0F);
                                    GlStateManager.rotate(-var151 * 20.0F, 0.0F, 0.0F, 1.0F);
                                    GlStateManager.rotate(-var151 * 80.0F, 1.0F, 0.0F, 0.0F);
                                    break;
                                case "Clean":
                                    transformFirstPersonItem(-0.25F, 1.0F + MathHelper.sin(MathHelper.sqrt_float(f1) * (float) Math.PI) / 10.0F);
                                    GL11.glRotated(-MathHelper.sin(MathHelper.sqrt_float(f1) * (float) Math.PI) * 50f, 1, 1, 1);
                                    this.doBlockTransformations();
                                    break;
                                case "Sigma":
                                ...
                                case "Jello":
                                    ...
                                    this.doBlockTransformations();
                                    GlStateManager.rotate(MathHelper.sin(MathHelper.sqrt_float(f1) * (float) Math.PI) * 35.0F / 2.0F, 0.0F, 1.0F, 1.5F);
                                    GlStateManager.rotate(-MathHelper.sin(MathHelper.sqrt_float(f1) * (float) Math.PI) * 135.0F / 4.0F, 1.0f, 1.0F, 0.0F);
                                    break;
                                }
                                break;
                ...
                if (Math.abs(Animations.scale.getValue() - 1) != 0) {
                    GlStateManager.scale(Animations.scale.getValue(), Animations.scale.getValue(), Animations.scale.getValue());
                }
    ...
    private void renderFireInFirstPerson(float partialTicks) {
        ...
        if (Ambient.getInstance().getModuleManager().getModule(Camera.class).nofire.getValue() && Ambient.getInstance().getModuleManager().getModule(Camera.class).isEnabled()) {
            return;
        }
