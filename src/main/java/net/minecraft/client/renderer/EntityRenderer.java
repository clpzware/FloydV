...
import fr.ambient.event.impl.render.SetMouseTargetEvent;
import fr.ambient.module.impl.combat.Reach;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.theWorld != null) {
            ...
            Reach reach = Ambient.getInstance().getModuleManager().getModule(Reach.class);
            ...
            if (this.mc.playerController.extendedReach() || reach.isEnabled()) {
                d0 = 6.0D;
                d1 = 6.0D;
                if(reach.isEnabled()){
                    d0 = reach.reachDistance.getValue();
                    d1 = reach.reachDistance.getValue();
                }
            } else if (d0 > 3.0D) {
                flag = true;
            }
            ...
            double maxDist = 3.0D;
            if(reach.isEnabled()){
                maxDist = reach.reachDistance.getValue();
            }
            if (this.pointedEntity != null && flag && vec3.distanceTo(vec33) > maxDist) {
                this.pointedEntity = null;
    ...
    private void orientCamera(float partialTicks) {
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                if(!(Ambient.getInstance().getModuleManager().getModule(Camera.class).trw.getValue() && Ambient.getInstance().getModuleManager().getModule(Camera.class).isEnabled())){
                    for (int i = 0; i < 8; ++i) {
        ...
        if (!this.mc.gameSettings.debugCamEnable) {
            ...
            float pitch = freelook.getRenderRotations()[3] + (freelook.getRenderRotations()[1] - freelook.getRenderRotations()[3]) * partialTicks;
            float yaw = freelook.getRenderRotations()[2] + (freelook.getRenderRotations()[0] - freelook.getRenderRotations()[2]) * partialTicks;

            EntityRenderEvent event = new EntityRenderEvent(0,0,0, yaw, pitch);

            Ambient.getInstance().getEventBus().post(event);

            yaw = (float) event.getYaw();
            pitch = (float) event.getPitch();
        ...
        if (this.lightmapUpdateNeeded) {
            ...
            if (world != null) {
                ...
                for (int i = 0; i < 256; ++i) {
                    ...
                    float f16 = Ambient.getInstance().getModuleManager().getModule(Camera.class).fullbright.getValue() && Ambient.getInstance().getModuleManager().getModule(Camera.class).isEnabled() ? 10000 : this.mc.gameSettings.gammaSetting;
                    float f17 = 1.0F - f8;
                    float f13 = 1.0F - f9;
                    float f14 = 1.0F - f10;
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime) {
        ...
        if (flag && Minecraft.isRunningOnMac && this.mc.inGameHasFocus && mc.currentScreen == null) {
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    if(!Objects.equals(Ambient.getInstance().getServerIp(), "")){
                        try {
                            String[] spt= Ambient.getInstance().getServerIp().split(":");
                            Minecraft.getMinecraft().displayGuiScreen(new GuiConnecting(null, Minecraft.getMinecraft(), new ServerData(spt[0], Ambient.getInstance().getServerIp(), false)));

                        }catch (Exception e){
                            e.printStackTrace();
                        }
                        Ambient.getInstance().setServerIp("");
                    }
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        ...
        Ambient.getInstance().getEventBus().post(new Render3DEvent(partialTicks));
