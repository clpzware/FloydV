...
import fr.ambient.module.impl.render.player.Chams;
import fr.ambient.module.impl.render.player.ESP;
import fr.ambient.module.impl.render.player.HurtColor;
import fr.ambient.module.impl.render.player.PlayerRender;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        Chams chams = Ambient.getInstance().getModuleManager().getModule(Chams.class);
        boolean useChams = entity instanceof EntityPlayer && !antiBot.isBot(entity) && chams.isEnabled();

        if (useChams) {
            GL42.glEnable(GL42.GL_POLYGON_OFFSET_FILL);
            GL42.glPolygonOffset(1.0f, -1000000);
        }
        ...
        try {
            ...
            entity.discoAnimation.run(1);

            if (entity.discoAnimation.isFinished()) {
                entity.discoAnimation.restart();
            }

            final PlayerRender playerRender = Ambient.getInstance().getModuleManager().getModule(PlayerRender.class);
            if (playerRender.isEnabled()) {
                double scaleX, scaleY, scaleZ;

                if (playerRender.scale.getValue()) {
                    scaleX = playerRender.xMult.getValue();
                    scaleY = playerRender.yMult.getValue();
                    scaleZ = playerRender.zMult.getValue();
                } else scaleX = scaleY = scaleZ = 1f;

                float modelHeight = 1.5078125F;
                GlStateManager.translate(0.0F, modelHeight, 0.0F);
    ...
    protected void renderModel(T entitylivingbaseIn, float p_77036_2_, float p_77036_3_, float p_77036_4_, float p_77036_5_, float p_77036_6_, float scaleFactor, float partialTicks) {
        boolean flag = !entitylivingbaseIn.isInvisible();
        boolean flag1 = !flag && !entitylivingbaseIn.isInvisibleToPlayer(Minecraft.getMinecraft().thePlayer);

        ESP esp = Ambient.getInstance().getModuleManager().getModule(ESP.class);
        boolean flag2 = !flag1 && !flag && esp.isEnabled() && esp.outlines.getValue();
        ...
        if (flag || flag1 || flag2) {
            ...
            EventBus eventBus = Ambient.getInstance().getEventBus();

            ModelRenderEvent event = new ModelRenderEvent(
                    entitylivingbaseIn,
                    () ->  this.mainModel.render(entitylivingbaseIn, p_77036_2_, p_77036_3_, p_77036_4_, p_77036_5_, p_77036_6_, scaleFactor),
                    () ->  this.renderLayers(entitylivingbaseIn, p_77036_2_, p_77036_3_, partialTicks, p_77036_4_, p_77036_5_, p_77036_6_, scaleFactor)
            );

            event.setStage(EventStage.PRE);
            eventBus.post(event);

            if (!flag2) event.drawEntityModel();
    ...
    protected boolean setBrightness(T entitylivingbaseIn, float partialTicks, boolean combineTextures) {
        ...
        if (!flag && !flag1) {
            ...
            if (flag1) {
                ...
                HurtColor hurtColor = Ambient.getInstance().getModuleManager().getModule(HurtColor.class);
                if(hurtColor.isEnabled()){
                    red = hurtColor.hurtColor.getValue().getRed() / 255f;
                    green = hurtColor.hurtColor.getValue().getGreen() / 255f;
                    blue = hurtColor.hurtColor.getValue().getBlue() / 255f;
                    alpha = hurtColor.transparency.getValue();
                }
