...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        Ambient.getInstance().getEventBus().post(new Render2DEvent(partialTicks));
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
    ...
    public void renderScoreboard(ScoreObjective p_180475_1_, int x, int y, Color color, boolean shadow)
    {
        ...
        int k1 = y + j1+ this.getFontRenderer().FONT_HEIGHT;
        byte b0 = 3;
        int j = x - b0;
        int k = 0;
        Color c = color;
        ...
        for (Object score1 : arraylist1)
        {
            ...
            int i1 = x - b0 + 2;
            RenderUtil.drawRect(j - 2, l, i1-(j - 2)+i, this.getFontRenderer().FONT_HEIGHT, c);
            ww += this.getFontRenderer().FONT_HEIGHT;
            if (shadow) {
                this.getFontRenderer().drawStringWithShadow(s1, j, l, 553648127);
                this.getFontRenderer().drawStringWithShadow(s2, i1 - this.getFontRenderer().getStringWidth(s2)+i, l, 553648127);
            } else {
                this.getFontRenderer().drawString(s1, j, l, 553648127);
                this.getFontRenderer().drawString(s2, i1 - this.getFontRenderer().getStringWidth(s2)+i, l, 553648127);
            }

            if (k == arraylist1.size())
            {
                String s3 = p_180475_1_.getDisplayName();
                RenderUtil.drawRect(j - 2, l - this.getFontRenderer().FONT_HEIGHT - 1, i1-(j - 2)+i, this.getFontRenderer().FONT_HEIGHT, c);
                ww += this.getFontRenderer().FONT_HEIGHT;
                RenderUtil.drawRect(j - 2, l - 1, i1-(j - 2)+i, 1, c);
                if (shadow) {
                    this.getFontRenderer().drawStringWithShadow(s3, j + i / 2 - this.getFontRenderer().getStringWidth(s3) / 2, l - this.getFontRenderer().FONT_HEIGHT, 553648127);
                } else {
                ...
                Ambient.getInstance().getModuleManager().getModule("Scoreboard").setWidth(i1-(j - 2)+i);
                Ambient.getInstance().getModuleManager().getModule("Scoreboard").setHeight(ww);
            }
        }
    }
}
