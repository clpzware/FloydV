...
import cheadleware.module.modules.Movement.Fly;
import cheadleware.module.modules.Movement.LongJump;
import cheadleware.module.modules.Movement.Speed;
...
public class TargetStrafe extends Module {
    ...
    private boolean canStrafe() {
        if (this.speedOnly.getValue()) {
            Speed speed = (Speed) Cheadleware.moduleManager.modules.get(Speed.class);
            Fly fly = (Fly) Cheadleware.moduleManager.modules.get(Fly.class);
            LongJump longJump = (LongJump) Cheadleware.moduleManager.modules.get(LongJump.class);
    ...
    private Color getTargetColor(EntityLivingBase entityLivingBase) {
        if (entityLivingBase instanceof EntityPlayer) {
            if (TeamUtil.isFriend((EntityPlayer) entityLivingBase)) {
                return Cheadleware.friendManager.getColor();
            }
            if (TeamUtil.isTarget((EntityPlayer) entityLivingBase)) {
                return Cheadleware.targetManager.getColor();
            }
        }
        switch (this.showTarget.getValue()) {
            ...
            case 2:
                int color = ((HUD) Cheadleware.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis()).getRGB();
