package cheadleware.module.modules.Combat;
...
import cheadleware.events.*;
import cheadleware.management.RotationState;
import cheadleware.mixin.IAccessorPlayerControllerMP;
import cheadleware.module.Module;
import cheadleware.module.modules.Misc.BedNuker;
import cheadleware.module.modules.Player.AutoHeal;
...
public class KillAura extends Module {
    ...
    private boolean performAttack(float yaw, float pitch) {
        if (!Cheadleware.playerStateManager.digging && !Cheadleware.playerStateManager.placing) {
    ...
    private boolean canAttack() {
        if (this.inventoryCheck.getValue() && mc.currentScreen instanceof GuiContainer) {
            ...
            if (((IAccessorPlayerControllerMP) mc.playerController).getIsHittingBlock()) {
                ...
                AutoHeal autoHeal = (AutoHeal) Cheadleware.moduleManager.modules.get(AutoHeal.class);
                if (autoHeal.isEnabled() && autoHeal.isSwitching()) {
                    return false;
                } else {
                    BedNuker bedNuker = (BedNuker) Cheadleware.moduleManager.modules.get(BedNuker.class);
                    if (bedNuker.isEnabled() && bedNuker.isReady()) {
                        return false;
                    } else if (Cheadleware.moduleManager.modules.get(Scaffold.class).isEnabled()) {
                        return false;
    ...
    public void onUpdate(UpdateEvent event) {
        if (event.getType() == EventType.POST && this.blinkReset) {
            ...
            Cheadleware.blinkManager.setBlinkState(false, BlinkModules.AUTO_BLOCK);
            Cheadleware.blinkManager.setBlinkState(true, BlinkModules.AUTO_BLOCK);
        }
                    ...
                    switch (this.autoBlock.getValue()) {
                        case 0:
                            if (PlayerUtil.isUsingItem()) {
                                ...
                                if (!this.isPlayerBlocking() && !Cheadleware.playerStateManager.digging && !Cheadleware.playerStateManager.placing) {
                                    swap = true;
                                }
                            } else {
                                this.isBlocking = false;
                        ...
                        case 2:
                            if (this.hasValidTarget()) {
                                ...
                                if (Cheadleware.playerStateManager.digging
                                        || Cheadleware.playerStateManager.placing
                        ...
                        case 5:
                            if (this.hasValidTarget()) {
                                int item = ((IAccessorPlayerControllerMP) mc.playerController).getCurrentPlayerItem();
                                if (mc.thePlayer.inventory.currentItem == item && !Cheadleware.playerStateManager.digging && !Cheadleware.playerStateManager.placing) {
                                    switch (this.blockTick) {
                                        case 0:
                                            if (!this.isPlayerBlocking()) {
                                                swap = true;
                        ...
                        case 8:
                            ...
                            if (PlayerUtil.isUsingItem()
                                    ...
                                    && !Cheadleware.playerStateManager.digging
                                    && !Cheadleware.playerStateManager.placing) {
                                swap = true;
                            }
                    }
                ...
                if (this.isBoxInSwingRange(this.target.getBox())) {
                    if (this.rotations.getValue() == 2 || this.rotations.getValue() == 3) {
                        ...
                        if (this.rotations.getValue() == 3) {
                            Cheadleware.rotationManager.setRotation(rotations[0], rotations[1], 1, true);
                        }
            ...
            if (this.showTarget.getValue() != 0
                ...
                switch (this.showTarget.getValue()) {
                    case 1:
                        ...
                        break;
                    case 2:
                        color = ((HUD) Cheadleware.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis());
                }
