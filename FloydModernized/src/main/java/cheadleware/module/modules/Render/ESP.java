...
import cheadleware.events.ResizeEvent;
import cheadleware.mixin.IAccessorEntityRenderer;
...
import cheadleware.util.shader.GlowShader;
import cheadleware.util.shader.OutlineShader;
    ...
    private Color getEntityColor(EntityPlayer entityPlayer) {
        if (TeamUtil.isFriend(entityPlayer)) {
            ...
            switch (this.color.getValue()) {
                ...
                case 2:
                    int hudColor = ((HUD) Cheadleware.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis()).getRGB();
