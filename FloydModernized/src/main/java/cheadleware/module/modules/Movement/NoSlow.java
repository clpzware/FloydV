...
import cheadleware.enums.FloatModules;
...
import cheadleware.events.RightClickMouseEvent;
import cheadleware.module.Module;
import cheadleware.util.BlockUtil;
...
public class NoSlow extends Module {
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        if (this.isEnabled() && this.isFloatMode()) {
            ...
            if (this.lastSlot != item && PlayerUtil.isUsingItem()) {
                this.lastSlot = item;
                Cheadleware.floatManager.setFloatState(true, FloatModules.NO_SLOW);
            }
        } else {
            this.lastSlot = -1;
            Cheadleware.floatManager.setFloatState(false, FloatModules.NO_SLOW);
        }
    }
    ...
    public void onRightClick(RightClickMouseEvent event) {
        if (this.isEnabled()) {
            ...
            if (this.isFloatMode() && !Cheadleware.floatManager.isPredicted() && mc.thePlayer.onGround) {
