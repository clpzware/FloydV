...
import cheadleware.command.CommandManager;
import cheadleware.command.commands.*;
import cheadleware.config.Config;
import cheadleware.event.EventManager;
import cheadleware.management.*;
...
import cheadleware.property.PropertyManager;
...
public class Cheadleware {
    public static String clientName = "§f[§5cheadleware§r§f] §f>> §r";
...
//        moduleManager.modules.put(Radio.class, new Radio());
        ...
        try {
            cheadleware.util.font.FontManager.initTextures();
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try (InputStreamReader reader = new InputStreamReader(Objects.requireNonNull(Cheadleware.class.getResourceAsStream("/mcmod.info")), StandardCharsets.UTF_8)) {
