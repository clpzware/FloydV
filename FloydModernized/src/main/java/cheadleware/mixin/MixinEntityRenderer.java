...
import cheadleware.data.Box;
import cheadleware.event.EventManager;
import cheadleware.events.PickEvent;
import cheadleware.events.RaytraceEvent;
import cheadleware.events.Render3DEvent;
import cheadleware.module.modules.Combat.KillAura;
import cheadleware.module.modules.Player.AntiDebuff;
import cheadleware.module.modules.Player.GhostHand;
import cheadleware.module.modules.Player.Scaffold;
import cheadleware.module.modules.Render.NoHurtCam;
...
public abstract class MixinEntityRenderer {
    ...
    private float hurtCameraEffect(float float1) {
        if (Cheadleware.moduleManager == null) {
            ...
            NoHurtCam noHurtCam = (NoHurtCam) Cheadleware.moduleManager.modules.get(NoHurtCam.class);
    ...
    private void a(
        ...
        if (Cheadleware.moduleManager != null) {
            GhostHand event = (GhostHand) Cheadleware.moduleManager.modules.get(GhostHand.class);
            if (event.isEnabled()) {
    ...
    private double v(Vec3 vec31, Vec3 vec32) {
        if (Cheadleware.moduleManager == null) {
            ...
            return Cheadleware.moduleManager.modules.get(ViewClip.class).isEnabled() ? (double) this.thirdPersonDistance : vec31.distanceTo(vec32);
        }
    }
    ...
    private Material x(Block block) {
        if (Cheadleware.moduleManager == null) {
            ...
            return Cheadleware.moduleManager.modules.get(ViewClip.class).isEnabled() ? Material.air : block.getMaterial();
        }
    }
    ...
    private boolean y(EntityLivingBase entityLivingBase, Potion potion) {
        if (potion == Potion.blindness && Cheadleware.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) Cheadleware.moduleManager.modules.get(AntiDebuff.class);
    ...
    private boolean c(EntityPlayerSP entityPlayerSP, Potion potion) {
        if (potion == Potion.confusion && Cheadleware.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) Cheadleware.moduleManager.modules.get(AntiDebuff.class);
